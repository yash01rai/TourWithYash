...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoieWFzaHJhaTAxIiwiYSI6ImNreHd5b25wZzF4ZnUyb3BjcGszcmozdnMifQ.gQaIrouUQMHFAnLq3E0JrQ';
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description)).addTo(map); // Extends map bounds to include current location
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            stripe = Stripe('pk_test_51KG0bUSAxOusrztZJtsYbgFsWGf6IYwHOZCAWEhW1DAHrNlD2aSVfOoXj3f9LcZEb6dlgU5iSl24vFcnfzIFogGs009ZbAhSsV'); // 1) Get checkout session from API

            _context.next = 4;
            return _axios.default.get("http://localhost:4000/api/v1/bookings/checkout-session/".concat(tourId));
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              (0, _alert.showAlert)('success', 'Sign up done successfully!');
              window.setTimeout(function () {
                return location.assign('/');
              }, 1500);
            }
...
if (signupForm) {
  signupForm.addEventListener('submit', function (e) {
    ...
    var email = document.getElementById('email').value;
    var password = document.getElementById('password').value;
    var passwordConfrim = document.getElementById('password-confirm').value; // console.log(name, email, password, passwordConfrim);
